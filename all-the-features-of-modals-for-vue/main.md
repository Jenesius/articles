...
![example open modal](./example-open-modal.gif)
...
Выглядит это следующим образом:
![example push modal](./example-push-modal.gif)
...
Выглядит это следующим образом:
![example-prompt-modal](./example-prompt-modal.gif)
...
Показать как это работает в gif не удалось бы. Как работает интеграция
с vue-router можно посмотреть на [sandbox](https://codesandbox.io/s/vue-modal-router-n9rn94).
