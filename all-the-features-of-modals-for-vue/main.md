...
Наверное самые популярный вопрос, который я встречал, т.к. предыдущие
...
веб-приложения с модальными окнами безопасной. Эта библиотека 
наследует эту концепцию. Модальное окно — это отдельный логический 
уровень, принимающий входные параметры и каким-то образом с ними 
взаимодействующий. Однако бывают случаи, когда модальное окно является
лишь частью процесса. 
...
Callback-и это круто, но для меня более удобным является линей код,
...
закрытие, пока пользователь не пример _"Согласие на обработку данных"_, 
переход на нужные route не должен происходить.

Реализуем базовую функцию-обёртку, которую будет передавать в `Router`, когда
...
Теперь при инициализации `route` будет оборачивать модальные окна этой
функцией:
...
const routes = [
    {
        ...
        children: [
            {
                path: ":user-id",
                component: useModalRouter(ModalUser)
            }
        ]
    }
]
...
При переходе на `/users/5` мы не будем ничего создавать и рендерить. 
...
router.afterEach(async (to) => {
    closeModal(); 
    const modalComponent = findModal(to);
...
- закрываем все модальные окна перед тем, как перейти на новый route
- Ищем модальную компоненту. Для этого мы реализовали функцию `findModal`:
...
- Для текущего маршрута получаем все совпадения описанные для текущего
маршрута в routes. 
- Получаем объект компонент, которые были указаны для рендеринга
- ищем среди них те, у которых установлен флаг `_isModal`.
...
function useModalRouter(component) {
	return {
        initialize() {
            const params = computed(() => state.router?.currentRoute.value.params);
...
работе с модальными окнами, большое количество тестов и описанная документация.
